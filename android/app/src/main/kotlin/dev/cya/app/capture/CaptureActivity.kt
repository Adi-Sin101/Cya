...
class CaptureActivity : Activity() {
    ...
    private data class Source(val label: String, val packageName: String?)
     ...
     *
     * The package name is stored alongside the label so the app can render that app's *real*
     * launcher icon in the promise list rather than an approximation of it.
     */
    private fun resolveSource(): Source {
        val packageName = callingPackage ?: referrer?.host
            ?: return Source(UNKNOWN_SOURCE, null)
        return try {
            val manager = packageManager
            val label = manager
                .getApplicationLabel(manager.getApplicationInfo(packageName, 0))
                .toString()
            Source(label, packageName)
        } catch (_: Exception) {
            // A package we cannot resolve is not one we can draw an icon for either.
            Source(UNKNOWN_SOURCE, null)
        }
    }
